...
# קבצי PAPER לכל שוק
CRYPTO_FILE = os.path.join(RESULTS_DIR, "crypto_paper_equity.csv")
US_FILE = os.path.join(RESULTS_DIR, "us_paper_equity.csv")
IL_FILE = os.path.join(RESULTS_DIR, "il_paper_equity.csv")
...
def get_texts(language: str) -> Dict[str, str]:
    ...
    if language == "HE":
        return {
            "app_title": "Multi-Asset Strategy Dashboard – Paper Trading",
            "app_caption": "Paper Trading 2022–2025, הון התחלתי לניתוח: 100,000$ לכל בוט.",
            ...
            "crypto_summary_title": "Crypto Summary (BTC only, Paper)",
            "us_summary_title": "US Summary (S&P500 only, Paper)",
            "il_summary_title": "Israel Summary (TA-125 only, Paper)",
            "multi_title": "Multi-Engine Portfolio (Paper)",
            "multi_equity_title": "Equity Curves – CRYPTO / US / IL / MULTI (Paper)",
            "no_backtest_files": "No Paper CSV files found in results_multi. Make sure you ran the paper scripts.",
            ...
            "footer_caption": (
                "Paper Trading only. Monetary figures are computed relative to $100,000 initial capital per bot."
            ),
            "curve_both": "Both",
            "crypto_name": "Crypto",
            "us_name": "US",
        ...
        return {
            "app_title": "Multi-Asset Strategy Dashboard – Paper Trading",
            "app_caption": "Paper Trading 2022–2025, analysis based on initial capital of $100,000 per bot.",
...
def build_date_range_ym_mode(
    ...
    with col1:
        start_year = st.selectbox("Start year" if language == "EN" else "שנת פתיחה",
                                  options=years, index=0, key="start_year")
        start_month = st.selectbox(
            "Start month" if language == "EN" else "חודש פתיחה",
            options=months,
            index=0,
    ...
    with col2:
        end_year = st.selectbox("End year" if language == "EN" else "שנת סגירה",
                                options=years, index=len(years) - 1, key="end_year")
        end_month = st.selectbox(
            "End month" if language == "EN" else "חודש סגירה",
            options=months,
            index=11,
            format_func=lambda m: month_names[m - 1],
            key="end_month",
...
def load_multi_summary() -> Optional[pd.DataFrame]:
    if not os.path.exists(MULTI_SUMMARY_FILE):
        return None
    try:
        df = pd.read_csv(MULTI_SUMMARY_FILE)
        return df
    except Exception:
        return None


def load_multi_equity_curves() -> Optional[pd.DataFrame]:
    crypto = load_csv_with_date(CRYPTO_FILE)
    us = load_csv_with_date(US_FILE)
    il_df = load_csv_with_date(IL_FILE)
    multi = load_csv_with_date(MULTI_EQUITY_FILE)

    if all(x is None or x.empty for x in [crypto, us, il_df, multi]):
        return None

    merged = None
    if crypto is not None and not crypto.empty:
        merged = crypto[["date", "equity"]].rename(columns={"equity": "CRYPTO"})
    if us is not None and not us.empty:
        df = us[["date", "equity"]].rename(columns={"equity": "US"})
        merged = df if merged is None else merged.merge(df, on="date", how="outer")
    if il_df is not None and not il_df.empty:
        df = il_df[["date", "equity"]].rename(columns={"equity": "IL"})
        merged = df if merged is None else merged.merge(df, on="date", how="outer")
    if multi is not None and not multi.empty:
        df = multi[["date", "equity"]].rename(columns={"equity": "MULTI"})
        merged = df if merged is None else merged.merge(df, on="date", how="outer")
...
def main() -> None:
    st.set_page_config(page_title="Multi-Asset Strategy Dashboard – Paper", layout="wide")
    ...
    # נטען דאטה קודם כדי לקבוע טווחים
    ...
    if multi_summary is not None and not multi_summary.empty:
        ...
        st.info("multi_asset_summary.csv לא נמצא או ריק – אין מה להציג.")

    st.markdown("")
    st.markdown(f"**{texts['multi_equity_title']}**")

    multi_curves = load_multi_equity_curves()
    if multi_curves is not None and not multi_curves.empty:
        st.line_chart(multi_curves, height=350, use_container_width=True)
    else:
        st.info("לא נמצאו עקומות הון עבור CRYPTO / US / IL / MULTI.")
